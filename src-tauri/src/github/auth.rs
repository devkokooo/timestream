use crate::auth::{
    cached_session, clear_session_cache, delete_secret, load_secret, remember_session,
    store_secret, ForgeUser, SessionWrite,
};
use crate::error::{AppError, Result};
use crate::github::error as github_error;
use serde::{Deserialize, Serialize};
use std::time::{SystemTime, UNIX_EPOCH};

pub const TOKEN_ACCOUNT: &str = "github-token";
pub const REFRESH_ACCOUNT: &str = "github-refresh";
pub const EXPIRES_ACCOUNT: &str = "github-token-expires";
...
/// GitHub `/user` JSON is snake_case; `ForgeUser` is camelCase for IPC.
#[derive(Deserialize)]
...
impl From<GithubApiUser> for ForgeUser {
...
fn github_user_from_api(user: GithubApiUser, extra: &[String]) -> ForgeUser {
...
pub async fn login_poll(device_code: &str) -> Result<Option<ForgeUser>> {
    if GITHUB_CLIENT_ID.is_empty() {
...
pub async fn login_pat(token: &str) -> Result<ForgeUser> {
...
pub async fn whoami() -> Result<Option<ForgeUser>> {
...
async fn list_user_emails(client: &reqwest::Client, token: &str) -> Vec<String> {
    let Ok(res) = github_authed_get(client, token, "https://api.github.com/user/emails").await
    else {
        return Vec::new();
    };
...
pub async fn whoami_with(token: &str) -> Result<ForgeUser> {
    let client = reqwest::Client::new();
...
#[tauri::command]
pub async fn github_login_poll(device_code: String) -> Result<Option<ForgeUser>> {
    login_poll(&device_code).await
}

#[tauri::command]
pub async fn github_login_pat(token: String) -> Result<ForgeUser> {
    login_pat(&token).await
}

#[tauri::command]
pub async fn github_whoami() -> Result<Option<ForgeUser>> {
    whoami().await
}
...
mod tests {
    ...
    fn github_user_decodes_snake_case_api_json() {
        let user: ForgeUser = serde_json::from_str::<GithubApiUser>(
            r#"{
                "login": "octocat",
                "name": "The Octocat",
    ...
    fn merge_identity_emails_dedupes_case() {
        let (primary, emails) = merge_identity_emails(
            ...
            &[
                "analyst@tva.local".into(),
                "noreply@users.noreply.github.com".into(),
            ],
        );
    ...
    #[test]
    fn keychain_account_names_stay_github() {
        assert_eq!(TOKEN_ACCOUNT, "github-token");
        assert_eq!(REFRESH_ACCOUNT, "github-refresh");
        assert_eq!(EXPIRES_ACCOUNT, "github-token-expires");
