...
export function identityEmails(user: ForgeUser): string[] {
...
export function isSelfEmail(user: ForgeUser | null | undefined, email?: string | null): boolean {
  if (!user || !email) return false;
  const needle = normalizeEmail(email);
  return Boolean(needle) && identityEmails(user).includes(needle);
}

export function isSelfLogin(user: ForgeUser | null | undefined, login?: string | null): boolean {
  if (!user || !login) return false;
  return user.login.toLowerCase() === login.trim().toLowerCase();
}

export function isSelfPerson(
  user: ForgeUser | null | undefined,
